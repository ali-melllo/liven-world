export const translations = {
  en: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Your multilingual guide to navigating life in the Netherlands.",
    signUp: "Sign Up",
    logIn: "Log In",
    logOut: "Log Out",
    termsText: "By continuing, you agree to our Terms of Service and Privacy Policy.",
    sendVerificationCode: "Send Verification Code",

    // Landing Page
    heroTitle: "Welcome to Liven World",
    heroSubtitle: "Your trusted multilingual assistant for migrants and asylum seekers in the Netherlands",
    heroDescription:
      "Get instant help with housing, healthcare, legal matters, and more - available 24/7 in your language.",
    getStartedNow: "Get Started Now",
    learnMore: "Learn More",

    // Features
    featuresTitle: "How We Help You",
    feature1Title: "24/7 Multilingual Support",
    feature1Desc: "Get help anytime in your preferred language",
    feature2Title: "Expert Guidance",
    feature2Desc: "Reliable information on legal, housing, and healthcare matters",
    feature3Title: "Easy to Use",
    feature3Desc: "Simple chat interface designed for everyone",

    // Welcome & Onboarding
    welcomeTitle: "Welcome to Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "Your multilingual guide to navigating life in Netherlands",
    welcomeSubtitle: "Your multilingual assistant for migrants and asylum seekers in the Netherlands.",
    introRule: "By continuing you agree to our Terms Of Service & Privacy Policy",
    getStarted: "Get Started",

    // Language Selection
    language: "Language",
    selectLanguage: "Select your language",
    continue: "Continue",
    back: "Back",

    // Profile Setup
    profileSetup: "Profile Setup",
    tellUsAbout: "Tell us about yourself",
    enterEmail: "Enter Your Email Address",
    profileLogin: "Profile Login",
    fullName: "Full Name",
    nationality: "Nationality",
    municipality: "Municipality",
    emailAddress: "Email Address",
    password: "Password",
    gender: "Gender",
    status: "Status",
    next: "Next",

    // Gender Options
    male: "Male",
    female: "Female",
    other: "Other",
    preferNotToSay: "Prefer not to say",

    // Status Options
    asylumSeeker: "Asylum Seeker",
    refugee: "Refugee",
    migrant: "Migrant",
    student: "Student",

    // Validation Messages
    required: "This field is required",
    fullNameRequired: "Full name is required",
    nationalityRequired: "Nationality is required",
    municipalityRequired: "Municipality is required",
    emailRequired: "Email is required",
    emailInvalid: "Please enter a valid email address",
    passwordRequired: "Password is required",
    passwordMinLength: "Password must be at least 6 characters",
    passwordMinLengthUpdate: "Password must be at least 8 characters",
    confirmPasswordRequired: "Please confirm your password",
    passwordsDoNotMatch: "Passwords do not match",
    genderRequired: "Please select your gender",
    statusRequired: "Please select your status",

    // Onboarding Instructions
    howToUseTitle: "How to use the chat",
    onboardingText1:
      "Ask questions in your language and get answers in the same language. You can ask about your rights, procedures, and services available to you.",
    onboardingText2:
      "The chatbot is available 24/7 and can answer questions in multiple languages. It's designed to provide quick and accurate information to help you navigate your situation in the Netherlands.",
    onboardingText3:
      "Remember, the chatbot is a tool to assist you. For complex or urgent matters, always seek advice from a legal professional or relevant authorities.",
    startChatting: "Start Chatting",

    // Guide Slides
    guides: [
      {
        title: "Ask Questions Anytime",
        description:
          "Get instant answers about housing, healthcare, legal matters, and daily life in the Netherlands - available 24/7 in your language.",
        icon: "💬",
      },
      {
        title: "Multiple Languages",
        description:
          "Chat in your preferred language. Our AI assistant understands and responds in English, Dutch, Arabic, Spanish, French, Turkish, Ukrainian, and Russian.",
        icon: "🌍",
      },
      {
        title: "Expert Information",
        description:
          "Access reliable, up-to-date information about Dutch laws, procedures, and services specifically designed for migrants and asylum seekers.",
        icon: "📚",
      },
      {
        title: "Privacy & Security",
        description:
          "Your conversations are private and secure. We protect your personal information and never share your data with third parties.",
        icon: "🔒",
      },
      {
        title: "Get Started",
        description:
          "Ready to begin? Tap 'Start Chatting' to ask your first question. You can always change your language settings in your profile.",
        icon: "🚀",
      },
    ],

    // Navigation
    home: "Home",
    chat: "Chat",
    topics: "Topics",
    profile: "Profile",

    // Chat
    startNewChat: "Start a new chat",
    welcomeToLivenChat: "Welcome to Liven Chat",
    chatDescriptionIntro: "I'm here to help you navigate life in the Netherlands. Ask me anything about housing, jobs, government services, or integration.",
    askQuestion: "Ask a question",
    askQuestionDesc: "Get answers to your questions about living in the Netherlands.",
    commonTopics: "Common topics",
    typeMessage: "Type your message...",
    chatHistory: "Chat History",
    chatLoading: "Thinking ...",
    agentConnect: "Connect With a Support Agent",
    findAnswers: "Find Answers to Frequently asked questions",

    // Topics
    housing: "Housing",
    housingDesc: "Find help with housing and rent",
    health: "Health",
    healthDesc: "Access healthcare and insurance",
    work: "Work",
    workDesc: "Jobs, permits and career guidance",
    legal: "Legal",
    legalDesc: "Residency, registration, and law",
    culture: "Culture",
    cultureDesc: "Traditions, language, and society",
    finance: "Finance",
    financeDesc: "Banking, taxes, and money management",
    education: "Education",
    educationDesc: "Schools, courses, and learning opportunities",

    // Profile
    joined: "Joined",
    account: "Account",
    email: "Email",
    settings: "Settings",
    notifications: "Notifications",
    privacy: "Privacy",
    help: "Help",
    about: "About",
    updateProfile: "Update Profile",
    changeLanguage: "Change Language",
    currentLanguage: "Current Language",
    selectNewLanguage: "Select New Language",
    save: "Save",
    cancel: "Cancel",
    updateEmail: "Update Email",
    updatePassword: "Update Password",
    currentEmail: "Current Email",
    newEmail: "New Email",
    currentPassword: "Current Password",
    newPassword: "New Password",
    confirmNewPassword: "Confirm New Password",
    profileUpdated: "Profile updated successfully",
    languageUpdated: "Language updated successfully",

    housing_q1: "How do I register at a municipality (BRP)?",
    housing_q2: "What documents do I need to rent an apartment?",
    housing_q3: "How can I find social housing?",
    housing_q4: "What are my rights if my landlord increases rent?",
    housing_q5: "How do I apply for housing allowance (huurtoeslag)?",

    legal_q1: "How do I apply for a residence permit?",
    legal_q2: "What is family reunification and how can I apply?",
    legal_q3: "What are my rights while my asylum case is pending?",
    legal_q4: "Where can I get free legal aid as a migrant?",

    work_q1: "Do I need a work permit (TWV)?",
    work_q2: "Where can I find vacancies for newcomers?",
    work_q3: "What are my rights as an employee in the Netherlands?",
    work_q4: "How do I register at UWV (employment office)?",
    work_q5: "Can I work while waiting for asylum decision?",

    healthcare_q1: "How do I register with a huisarts (GP)?",
    healthcare_q2: "Do I need health insurance, and how do I get it?",
    healthcare_q3: "What is covered under Dutch basic health insurance?",
    healthcare_q4: "How do I apply for healthcare allowance (zorgtoeslag)?",

    education_q1: "How do I enroll my child in school?",
    education_q2: "What are my options for Dutch language courses?",
    education_q3: "Can I study at a Dutch university as a newcomer?",
    education_q4: "Are there scholarships or financial support for migrants?",

    culture_q1: "What are some Dutch cultural traditions?",
    culture_q2: "Where can I learn the Dutch language?",
    culture_q3: "How do Dutch holidays differ from mine?",

    finance_q1: "How do I open a Dutch bank account?",
    finance_q2: "What is the process for getting a BSN number?",
    finance_q3: "How do I pay taxes in the Netherlands?",

    // Theme
    theme: "Theme",
    light: "Light",
    dark: "Dark",
    system: "System",



    // new 

    myHeroTitle: "My Hero",
    myHeroDescription: "My Hero connects migrants and locals. You can post if you need help or want to offer help. Follow safety guidelines: be respectful, no discrimination, no spam. Moderation is active: report inappropriate posts.",

    helpRequests: "Help Requests",
    helpRequestsDesc: "Post here if you need assistance with everyday tasks, guidance, or support from the community.",

    helpOffers: "Help Offers",
    helpOffersDesc: "Post here if you want to offer help, resources, or support to migrants and community members.",

    privacyPolicyTitle: "Privacy Policy",
    privacyIntroductionTitle: "Introduction",
    privacyIntroductionDesc: "Liven Chat and My Hero value your privacy. This policy explains what data we collect, how we use it, and your rights.",

    dataCollectionTitle: "Data Collection",
    dataCollectionDesc: "We collect the following types of data:",
    dataCollectionUsers: "User profile information (name, email, nationality, municipality).",
    dataCollectionPosts: "Posts and comments you create within the app.",
    dataCollectionComments: "Comments on posts, including content and timestamps.",
    dataCollectionPush: "Push notification subscriptions to deliver real-time updates.",

    dataUseTitle: "How We Use Your Data",
    dataUseDesc: "Your data is used to improve your experience and provide our services, including:",
    dataUseNotifications: "Sending real-time notifications about likes, comments, and community activity.",
    dataUsePersonalization: "Personalizing your feed and content recommendations.",
    dataUseAnalytics: "Analyzing app usage to improve functionality and features.",

    dataSharingTitle: "Data Sharing",
    dataSharingDesc: "We do not sell your personal data. We may share information with service providers to maintain and improve the app.",

    securityTitle: "Data Security",
    securityDesc: "We implement reasonable technical and organizational measures to protect your data from unauthorized access.",

    userRightsTitle: "Your Rights",
    userRightsDesc: "You have the following rights regarding your data:",
    userRightsAccess: "Access the personal data we hold about you.",
    userRightsDelete: "Request deletion of your data from our systems.",
    userRightsOptOut: "Opt out of receiving push notifications.",

    contactTitle: "Contact Us",
    contactDesc: "If you have any questions about this Privacy Policy, please contact us at support@livenchat.com.",

    health_q1: "How do I register for health insurance?",
    health_q1_answer: "You must register with a Dutch health insurance provider within four months of arriving in the Netherlands. You can do this online or at the provider’s office.",

    health_q2: "Where can I find a doctor near me?",
    health_q2_answer: "You can find a local huisarts (general practitioner) by searching online, asking your municipality, or checking the zorgkaart.nl website.",

    health_q3: "What should I do in case of a medical emergency?",
    health_q3_answer: "In case of a medical emergency, call 112 immediately. For urgent care, visit your local huisartsenpost or emergency department at a hospital.",

    health_q4: "Do I need health insurance, and how do I get it?",
    health_q4_answer: "Yes, health insurance is mandatory in the Netherlands. Choose a Dutch health insurance provider and register online or in person.",

    "faqTitle": "Frequently Asked Questions",
    "myHero": "My Hero",
    "howToPost": "How do I post a help request or offer?",
    "howToPostDesc": "You can create a post by clicking 'New Post', select your type, write the title and content, and publish.",
    "helpRequestsGuidelines": "Are there guidelines for posts?",
    "helpRequestsGuidelinesDesc": "Be respectful, no discrimination, no spam. Follow safety guidelines when posting.",
    "moderationRules": "How is content moderated?",
    "moderationRulesDesc": "You can report inappropriate posts. Our moderators will review and take action if necessary.",
    "privacyAndSafety": "How is my privacy protected?",
    "privacyAndSafetyDesc": "We store minimal information and never share your data. Follow safety rules while interacting with others.",
    "howToGetNotified": "How do I receive notifications?",
    "howToGetNotifiedDesc": "Enable push notifications in your browser or app to get updates about likes, comments, and new posts.",
    "likeAndCommentNotifications": "What notifications will I receive?",
    "likeAndCommentNotificationsDesc": "You will be notified when someone likes or comments on your posts.",
    "updateProfileDesc": "Go to Profile > Settings to update your personal information, avatar, or preferences.",
    "changeLanguageDesc": "You can select a different language in Profile > Change Language.",
    
    aboutTitle: "About the App",
    aboutDescription: "This app connects migrants and locals, helping you navigate life in your new country. You can ask for help, offer support, or access useful information.",
    aboutFeatureChatTitle: "Multilingual Chat",
    aboutFeatureChatDesc: "Chat in your preferred language with an AI assistant to get guidance on housing, health, legal matters, and more.",
    aboutFeatureHelpTitle: "Help Requests & Offers",
    aboutFeatureHelpDesc: "Post requests if you need help or offer assistance to others in your community.",
    aboutSafetyTitle: "Safety & Moderation",
    aboutSafetyDesc: "Be respectful, no discrimination, no spam. Report inappropriate posts. Moderators ensure safety and enforce guidelines."
    
  },
  "nl": {
    // Landing & Auth
    "appTitle": "Liven Chat",
    "appSubtitle": "Jouw meertalige gids om het leven in Nederland te navigeren.",
    "signUp": "Registreren",
    "logIn": "Inloggen",
    "logOut": "Uitloggen",
    "termsText": "Door verder te gaan, ga je akkoord met onze Servicevoorwaarden en Privacybeleid.",
    "sendVerificationCode": "Verificatiecode verzenden",

    // Landing Page
    "heroTitle": "Welkom bij Liven World",
    "heroSubtitle": "Jouw vertrouwde meertalige assistent voor migranten en asielzoekers in Nederland",
    "heroDescription":
      "Krijg direct hulp met huisvesting, gezondheidszorg, juridische zaken en meer - 24/7 beschikbaar in jouw taal.",
    "getStartedNow": "Begin nu",
    "learnMore": "Meer informatie",

    // Features
    "featuresTitle": "Hoe wij je helpen",
    "feature1Title": "24/7 Meertalige Ondersteuning",
    "feature1Desc": "Krijg altijd hulp in jouw voorkeurstaal",
    "feature2Title": "Deskundige Begeleiding",
    "feature2Desc": "Betrouwbare informatie over juridische, huisvestings- en gezondheidszorgzaken",
    "feature3Title": "Makkelijk te Gebruiken",
    "feature3Desc": "Eenvoudige chatinterface ontworpen voor iedereen",

    // Welcome & Onboarding
    "welcomeTitle": "Welkom bij Liven Chat",
    "introTitle": "Liven Chat",
    "welcomeIntro": "Jouw meertalige gids om het leven in Nederland te navigeren",
    "welcomeSubtitle": "Jouw meertalige assistent voor migranten en asielzoekers in Nederland.",
    "introRule": "Door verder te gaan ga je akkoord met onze Servicevoorwaarden en Privacybeleid",
    "getStarted": "Beginnen",

    // Language Selection
    "language": "Taal",
    "selectLanguage": "Selecteer je taal",
    "continue": "Doorgaan",
    "back": "Terug",

    // Profile Setup
    "profileSetup": "Profielinstellingen",
    "tellUsAbout": "Vertel ons over jezelf",
    "enterEmail": "Voer je e-mailadres in",
    "profileLogin": "Profiel Inloggen",
    "fullName": "Volledige naam",
    "nationality": "Nationaliteit",
    "municipality": "Gemeente",
    "emailAddress": "E-mailadres",
    "password": "Wachtwoord",
    "gender": "Geslacht",
    "status": "Status",
    "next": "Volgende",

    // Gender Options
    "male": "Man",
    "female": "Vrouw",
    "other": "Anders",
    "preferNotToSay": "Wil ik niet zeggen",

    // Status Options
    "asylumSeeker": "Asielzoeker",
    "refugee": "Vluchteling",
    "migrant": "Migrant",
    "student": "Student",

    // Validation Messages
    "required": "Dit veld is verplicht",
    "fullNameRequired": "Volledige naam is verplicht",
    "nationalityRequired": "Nationaliteit is verplicht",
    "municipalityRequired": "Gemeente is verplicht",
    "emailRequired": "E-mailadres is verplicht",
    "emailInvalid": "Voer een geldig e-mailadres in",
    "passwordRequired": "Wachtwoord is verplicht",
    "passwordMinLength": "Wachtwoord moet minimaal 6 tekens bevatten",
    "passwordMinLengthUpdate": "Wachtwoord moet minimaal 8 tekens bevatten",
    "confirmPasswordRequired": "Bevestig je wachtwoord",
    "passwordsDoNotMatch": "Wachtwoorden komen niet overeen",
    "genderRequired": "Selecteer je geslacht",
    "statusRequired": "Selecteer je status",

    // Onboarding Instructions
    "howToUseTitle": "Hoe de chat te gebruiken",
    "onboardingText1":
      "Stel vragen in jouw taal en krijg antwoorden in dezelfde taal. Je kunt vragen stellen over je rechten, procedures en diensten die voor jou beschikbaar zijn.",
    "onboardingText2":
      "De chatbot is 24/7 beschikbaar en kan vragen beantwoorden in meerdere talen. Het is ontworpen om snelle en accurate informatie te geven om je te helpen in Nederland.",
    "onboardingText3":
      "Onthoud: de chatbot is een hulpmiddel om je te ondersteunen. Voor complexe of dringende zaken, zoek altijd advies bij een juridisch professional of relevante autoriteiten.",
    "startChatting": "Begin met chatten",

    // Guide Slides
    "guides": [
      {
        "title": "Stel altijd vragen",
        "description":
          "Krijg direct antwoorden over huisvesting, gezondheidszorg, juridische zaken en dagelijks leven in Nederland - 24/7 beschikbaar in jouw taal.",
        "icon": "💬"
      },
      {
        "title": "Meerdere Talen",
        "description":
          "Chat in jouw voorkeurstaal. Onze AI-assistent begrijpt en antwoordt in Engels, Nederlands, Arabisch, Spaans, Frans, Turks, Oekraïens en Russisch.",
        "icon": "🌍"
      },
      {
        "title": "Deskundige Informatie",
        "description":
          "Toegang tot betrouwbare, actuele informatie over Nederlandse wetten, procedures en diensten, speciaal voor migranten en asielzoekers.",
        "icon": "📚"
      },
      {
        "title": "Privacy & Veiligheid",
        "description":
          "Je gesprekken zijn privé en veilig. Wij beschermen je persoonlijke informatie en delen je gegevens nooit met derden.",
        "icon": "🔒"
      },
      {
        "title": "Beginnen",
        "description":
          "Klaar om te starten? Tik op 'Begin met chatten' om je eerste vraag te stellen. Je kunt je taalinstellingen altijd wijzigen in je profiel.",
        "icon": "🚀"
      }
    ],

    // Navigation
    "home": "Home",
    "chat": "Chat",
    "topics": "Onderwerpen",
    "profile": "Profiel",

    // Chat
    "startNewChat": "Start een nieuw gesprek",
    "welcomeToLivenChat": "Welkom bij Liven Chat",
    "chatDescriptionIntro": "Ik ben hier om je te helpen het leven in Nederland te navigeren. Stel me vragen over huisvesting, werk, overheidsdiensten of integratie.",
    "askQuestion": "Stel een vraag",
    "askQuestionDesc": "Krijg antwoorden op je vragen over wonen in Nederland.",
    "commonTopics": "Veelgestelde onderwerpen",
    "typeMessage": "Typ je bericht...",
    "chatHistory": "Gespreksgeschiedenis",
    "chatLoading": "Denkt na ...",
    "agentConnect": "Verbinden met een supportmedewerker",
    "findAnswers": "Vind antwoorden op veelgestelde vragen",

    // Topics
    "housing": "Huisvesting",
    "housingDesc": "Hulp bij huisvesting en huur",
    "health": "Gezondheid",
    "healthDesc": "Toegang tot zorg en verzekering",
    "work": "Werk",
    "workDesc": "Banen, vergunningen en loopbaanbegeleiding",
    "legal": "Juridisch",
    "legalDesc": "Verblijf, registratie en recht",
    "culture": "Cultuur",
    "cultureDesc": "Tradities, taal en samenleving",
    "finance": "Financiën",
    "financeDesc": "Banken, belastingen en geldzaken",
    "education": "Onderwijs",
    "educationDesc": "Scholen, cursussen en leermogelijkheden",

    // Profile
    "joined": "Lid sinds",
    "account": "Account",
    "email": "E-mail",
    "settings": "Instellingen",
    "notifications": "Meldingen",
    "privacy": "Privacy",
    "help": "Hulp",
    "about": "Over",
    "updateProfile": "Profiel bijwerken",
    "changeLanguage": "Taal wijzigen",
    "currentLanguage": "Huidige taal",
    "selectNewLanguage": "Selecteer een nieuwe taal",
    "save": "Opslaan",
    "cancel": "Annuleren",
    "updateEmail": "E-mail bijwerken",
    "updatePassword": "Wachtwoord bijwerken",
    "currentEmail": "Huidig e-mailadres",
    "newEmail": "Nieuw e-mailadres",
    "currentPassword": "Huidig wachtwoord",
    "newPassword": "Nieuw wachtwoord",
    "confirmNewPassword": "Bevestig nieuw wachtwoord",
    "profileUpdated": "Profiel succesvol bijgewerkt",
    "languageUpdated": "Taal succesvol bijgewerkt",

    // Questions
    "housing_q1": "Hoe registreer ik mij bij een gemeente (BRP)?",
    "housing_q2": "Welke documenten heb ik nodig om een appartement te huren?",
    "housing_q3": "Hoe kan ik sociale huurwoningen vinden?",
    "housing_q4": "Wat zijn mijn rechten als mijn verhuurder de huur verhoogt?",
    "housing_q5": "Hoe vraag ik huurtoeslag aan?",

    "legal_q1": "Hoe vraag ik een verblijfsvergunning aan?",
    "legal_q2": "Wat is gezinshereniging en hoe kan ik dat aanvragen?",
    "legal_q3": "Wat zijn mijn rechten terwijl mijn asielzaak loopt?",
    "legal_q4": "Waar kan ik gratis juridische hulp krijgen als migrant?",

    "work_q1": "Heb ik een werkvergunning (TWV) nodig?",
    "work_q2": "Waar kan ik vacatures vinden voor nieuwkomers?",
    "work_q3": "Wat zijn mijn rechten als werknemer in Nederland?",
    "work_q4": "Hoe registreer ik mij bij het UWV (werkplein)?",
    "work_q5": "Mag ik werken terwijl ik op een asielbeslissing wacht?",

    "healthcare_q1": "Hoe schrijf ik mij in bij een huisarts (GP)?",
    "healthcare_q2": "Heb ik een zorgverzekering nodig, en hoe krijg ik die?",
    "healthcare_q3": "Wat valt onder de Nederlandse basisverzekering?",
    "healthcare_q4": "Hoe vraag ik zorgtoeslag aan?",

    "education_q1": "Hoe schrijf ik mijn kind in op school?",
    "education_q2": "Wat zijn mijn opties voor Nederlandse taalcursussen?",
    "education_q3": "Kan ik als nieuwkomer studeren aan een Nederlandse universiteit?",
    "education_q4": "Zijn er beurzen of financiële steun voor migranten?",

    "culture_q1": "Wat zijn enkele Nederlandse culturele tradities?",
    "culture_q2": "Waar kan ik de Nederlandse taal leren?",
    "culture_q3": "Hoe verschillen Nederlandse feestdagen van de mijne?",

    "finance_q1": "Hoe open ik een Nederlandse bankrekening?",
    "finance_q2": "Wat is de procedure voor het verkrijgen van een BSN-nummer?",
    "finance_q3": "Hoe betaal ik belasting in Nederland?",

    // Theme
    "theme": "Thema",
    "light": "Licht",
    "dark": "Donker",
    "system": "Systeem"
  },
  ar: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "دليلك متعدد اللغات للتنقل في الحياة في هولندا.",
    signUp: "إنشاء حساب",
    logIn: "تسجيل الدخول",
    logOut: "تسجيل الخروج",
    termsText: "بالمتابعة، فإنك توافق على شروط الخدمة وسياسة الخصوصية الخاصة بنا.",
    sendVerificationCode: "إرسال رمز التحقق",

    // Landing Page
    heroTitle: "مرحبًا بك في Liven World",
    heroSubtitle: "مساعدك متعدد اللغات للمهاجرين وطالبي اللجوء في هولندا",
    heroDescription:
      "احصل على مساعدة فورية في السكن، الرعاية الصحية، الأمور القانونية والمزيد - متاح 24/7 بلغتك.",
    getStartedNow: "ابدأ الآن",
    learnMore: "اعرف المزيد",

    // Features
    featuresTitle: "كيف نساعدك",
    feature1Title: "دعم متعدد اللغات على مدار الساعة",
    feature1Desc: "احصل على المساعدة في أي وقت بلغتك المفضلة",
    feature2Title: "إرشاد من خبراء",
    feature2Desc: "معلومات موثوقة حول القوانين والسكن والرعاية الصحية",
    feature3Title: "سهل الاستخدام",
    feature3Desc: "واجهة دردشة بسيطة مصممة للجميع",

    // Welcome & Onboarding
    welcomeTitle: "مرحبًا بك في Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "دليلك متعدد اللغات للتنقل في الحياة في هولندا",
    welcomeSubtitle: "مساعدك متعدد اللغات للمهاجرين وطالبي اللجوء في هولندا.",
    introRule: "بالمتابعة أنت توافق على شروط الخدمة وسياسة الخصوصية",
    getStarted: "ابدأ",

    // Language Selection
    language: "اللغة",
    selectLanguage: "اختر لغتك",
    continue: "متابعة",
    back: "رجوع",

    // Profile Setup
    profileSetup: "إعداد الملف الشخصي",
    tellUsAbout: "أخبرنا عن نفسك",
    enterEmail: "أدخل بريدك الإلكتروني",
    profileLogin: "تسجيل الدخول للملف الشخصي",
    fullName: "الاسم الكامل",
    nationality: "الجنسية",
    municipality: "البلدية",
    emailAddress: "البريد الإلكتروني",
    password: "كلمة المرور",
    gender: "الجنس",
    status: "الحالة",
    next: "التالي",

    // Gender Options
    male: "ذكر",
    female: "أنثى",
    other: "آخر",
    preferNotToSay: "أفضل عدم الإفصاح",

    // Status Options
    asylumSeeker: "طالب لجوء",
    refugee: "لاجئ",
    migrant: "مهاجر",
    student: "طالب",

    // Validation Messages
    required: "هذا الحقل مطلوب",
    fullNameRequired: "الاسم الكامل مطلوب",
    nationalityRequired: "الجنسية مطلوبة",
    municipalityRequired: "البلدية مطلوبة",
    emailRequired: "البريد الإلكتروني مطلوب",
    emailInvalid: "يرجى إدخال بريد إلكتروني صالح",
    passwordRequired: "كلمة المرور مطلوبة",
    passwordMinLength: "يجب أن تكون كلمة المرور 6 أحرف على الأقل",
    passwordMinLengthUpdate: "يجب أن تكون كلمة المرور 8 أحرف على الأقل",
    confirmPasswordRequired: "يرجى تأكيد كلمة المرور",
    passwordsDoNotMatch: "كلمات المرور غير متطابقة",
    genderRequired: "يرجى تحديد الجنس",
    statusRequired: "يرجى تحديد الحالة",

    // Onboarding Instructions
    howToUseTitle: "كيفية استخدام الدردشة",
    onboardingText1:
      "اطرح الأسئلة بلغتك واحصل على الإجابات بنفس اللغة. يمكنك أن تسأل عن حقوقك والإجراءات والخدمات المتاحة لك.",
    onboardingText2:
      "الروبوت متاح 24/7 ويمكنه الإجابة بلغات متعددة. تم تصميمه لتقديم معلومات سريعة ودقيقة لمساعدتك في التكيف في هولندا.",
    onboardingText3:
      "تذكر أن الروبوت أداة لمساعدتك فقط. في القضايا المعقدة أو العاجلة، استشر دائمًا محاميًا أو السلطات المختصة.",
    startChatting: "ابدأ الدردشة",

    // Guide Slides
    guides: [
      {
        title: "اطرح الأسئلة في أي وقت",
        description:
          "احصل على إجابات فورية حول السكن، الرعاية الصحية، الشؤون القانونية والحياة اليومية في هولندا - متاح 24/7 بلغتك.",
        icon: "💬",
      },
      {
        title: "لغات متعددة",
        description:
          "تحدث بلغتك المفضلة. مساعدنا الذكي يفهم ويجيب بالإنجليزية، الهولندية، العربية، الإسبانية، الفرنسية، التركية، الأوكرانية والروسية.",
        icon: "🌍",
      },
      {
        title: "معلومات موثوقة",
        description:
          "الوصول إلى معلومات حديثة وموثوقة حول القوانين والإجراءات والخدمات في هولندا المخصصة للمهاجرين وطالبي اللجوء.",
        icon: "📚",
      },
      {
        title: "الخصوصية والأمان",
        description:
          "محادثاتك خاصة وآمنة. نحن نحمي بياناتك الشخصية ولا نشاركها أبدًا مع أي طرف ثالث.",
        icon: "🔒",
      },
      {
        title: "ابدأ الآن",
        description:
          "هل أنت جاهز للبدء؟ اضغط على 'ابدأ الدردشة' لطرح سؤالك الأول. يمكنك دائمًا تغيير إعدادات اللغة في ملفك الشخصي.",
        icon: "🚀",
      },
    ],

    // Navigation
    home: "الرئيسية",
    chat: "دردشة",
    topics: "المواضيع",
    profile: "الملف الشخصي",

    // Chat
    startNewChat: "ابدأ محادثة جديدة",
    welcomeToLivenChat: "مرحبًا بك في Liven Chat",
    chatDescriptionIntro: "أنا هنا لمساعدتك في حياتك في هولندا. اسألني عن السكن، العمل، الخدمات الحكومية أو الاندماج.",
    askQuestion: "اطرح سؤالاً",
    askQuestionDesc: "احصل على إجابات لأسئلتك حول الحياة في هولندا.",
    commonTopics: "مواضيع شائعة",
    typeMessage: "اكتب رسالتك...",
    chatHistory: "سجل المحادثات",
    chatLoading: "جارٍ التفكير...",
    agentConnect: "التواصل مع وكيل دعم",
    findAnswers: "ابحث عن إجابات للأسئلة الشائعة",

    // Topics
    housing: "السكن",
    housingDesc: "مساعدة في السكن والإيجار",
    health: "الصحة",
    healthDesc: "الوصول إلى الرعاية الصحية والتأمين",
    work: "العمل",
    workDesc: "وظائف، تصاريح وإرشادات مهنية",
    legal: "القانون",
    legalDesc: "الإقامة، التسجيل، والقوانين",
    culture: "الثقافة",
    cultureDesc: "التقاليد، اللغة، والمجتمع",
    finance: "المالية",
    financeDesc: "المصارف، الضرائب، وإدارة الأموال",
    education: "التعليم",
    educationDesc: "المدارس، الدورات، وفرص التعلم",

    // Profile
    joined: "انضم منذ",
    account: "الحساب",
    email: "البريد الإلكتروني",
    settings: "الإعدادات",
    notifications: "الإشعارات",
    privacy: "الخصوصية",
    help: "المساعدة",
    about: "حول",
    updateProfile: "تحديث الملف الشخصي",
    changeLanguage: "تغيير اللغة",
    currentLanguage: "اللغة الحالية",
    selectNewLanguage: "اختر لغة جديدة",
    save: "حفظ",
    cancel: "إلغاء",
    updateEmail: "تحديث البريد الإلكتروني",
    updatePassword: "تحديث كلمة المرور",
    currentEmail: "البريد الإلكتروني الحالي",
    newEmail: "البريد الإلكتروني الجديد",
    currentPassword: "كلمة المرور الحالية",
    newPassword: "كلمة المرور الجديدة",
    confirmNewPassword: "تأكيد كلمة المرور الجديدة",
    profileUpdated: "تم تحديث الملف الشخصي بنجاح",
    languageUpdated: "تم تحديث اللغة بنجاح",

    // Housing
    housing_q1: "كيف أسجل في البلدية (BRP)؟",
    housing_q2: "ما هي المستندات المطلوبة لاستئجار شقة؟",
    housing_q3: "كيف يمكنني العثور على سكن اجتماعي؟",
    housing_q4: "ما هي حقوقي إذا زاد المالك الإيجار؟",
    housing_q5: "كيف أقدم طلب للحصول على بدل السكن (huurtoeslag)؟",

    // Legal
    legal_q1: "كيف أقدم طلب للحصول على تصريح إقامة؟",
    legal_q2: "ما هو لم الشمل العائلي وكيف يمكنني التقديم؟",
    legal_q3: "ما هي حقوقي أثناء انتظار قرار اللجوء؟",
    legal_q4: "أين يمكنني الحصول على مساعدة قانونية مجانية كمهاجر؟",

    // Work
    work_q1: "هل أحتاج إلى تصريح عمل (TWV)؟",
    work_q2: "أين يمكنني العثور على وظائف للمهاجرين الجدد؟",
    work_q3: "ما هي حقوقي كعامل في هولندا؟",
    work_q4: "كيف أسجل في مكتب التوظيف (UWV)؟",
    work_q5: "هل يمكنني العمل أثناء انتظار قرار اللجوء؟",

    // Healthcare
    healthcare_q1: "كيف أسجل لدى طبيب عام (huisarts)؟",
    healthcare_q2: "هل أحتاج إلى تأمين صحي، وكيف أحصل عليه؟",
    healthcare_q3: "ما الذي يغطيه التأمين الصحي الأساسي في هولندا؟",
    healthcare_q4: "كيف أقدم طلب للحصول على بدل الرعاية الصحية (zorgtoeslag)؟",

    // Education
    education_q1: "كيف أسجل طفلي في المدرسة؟",
    education_q2: "ما هي خياراتي لدورات اللغة الهولندية؟",
    education_q3: "هل يمكنني الدراسة في جامعة هولندية كمهاجر جديد؟",
    education_q4: "هل توجد منح دراسية أو دعم مالي للمهاجرين؟",

    // Culture
    culture_q1: "ما هي بعض التقاليد الثقافية الهولندية؟",
    culture_q2: "أين يمكنني تعلم اللغة الهولندية؟",
    culture_q3: "كيف تختلف الأعياد الهولندية عن أعيادي؟",

    // Finance
    finance_q1: "كيف أفتح حسابًا مصرفيًا هولنديًا؟",
    finance_q2: "ما هي إجراءات الحصول على رقم BSN؟",
    finance_q3: "كيف أدفع الضرائب في هولندا؟",

    // Theme
    theme: "المظهر",
    light: "فاتح",
    dark: "داكن",
    system: "النظام",
  },
  es: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Tu guía multilingüe para vivir en los Países Bajos.",
    signUp: "Registrarse",
    logIn: "Iniciar sesión",
    logOut: "Cerrar sesión",
    termsText: "Al continuar, aceptas nuestros Términos de servicio y Política de privacidad.",
    sendVerificationCode: "Enviar código de verificación",

    // Landing Page
    heroTitle: "Bienvenido a Liven World",
    heroSubtitle: "Tu asistente multilingüe de confianza para migrantes y solicitantes de asilo en los Países Bajos",
    heroDescription:
      "Obtén ayuda instantánea sobre vivienda, salud, asuntos legales y más, disponible 24/7 en tu idioma.",
    getStartedNow: "Comenzar ahora",
    learnMore: "Aprender más",

    // Features
    featuresTitle: "Cómo te ayudamos",
    feature1Title: "Soporte multilingüe 24/7",
    feature1Desc: "Obtén ayuda en cualquier momento en tu idioma preferido",
    feature2Title: "Orientación experta",
    feature2Desc: "Información confiable sobre asuntos legales, de vivienda y de salud",
    feature3Title: "Fácil de usar",
    feature3Desc: "Interfaz de chat simple diseñada para todos",

    // Welcome & Onboarding
    welcomeTitle: "Bienvenido a Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "Tu guía multilingüe para la vida en los Países Bajos",
    welcomeSubtitle: "Tu asistente multilingüe para migrantes y solicitantes de asilo en los Países Bajos.",
    introRule: "Al continuar aceptas nuestros Términos de servicio y Política de privacidad",
    getStarted: "Comenzar",

    // Language Selection
    language: "Idioma",
    selectLanguage: "Selecciona tu idioma",
    continue: "Continuar",
    back: "Atrás",

    // Profile Setup
    profileSetup: "Configuración del perfil",
    tellUsAbout: "Cuéntanos sobre ti",
    enterEmail: "Introduce tu correo electrónico",
    profileLogin: "Inicio de sesión de perfil",
    fullName: "Nombre completo",
    nationality: "Nacionalidad",
    municipality: "Municipio",
    emailAddress: "Correo electrónico",
    password: "Contraseña",
    gender: "Género",
    status: "Estado",
    next: "Siguiente",

    // Gender Options
    male: "Hombre",
    female: "Mujer",
    other: "Otro",
    preferNotToSay: "Prefiero no decirlo",

    // Status Options
    asylumSeeker: "Solicitante de asilo",
    refugee: "Refugiado",
    migrant: "Migrante",
    student: "Estudiante",

    // Validation Messages
    required: "Este campo es obligatorio",
    fullNameRequired: "El nombre completo es obligatorio",
    nationalityRequired: "La nacionalidad es obligatoria",
    municipalityRequired: "El municipio es obligatorio",
    emailRequired: "El correo electrónico es obligatorio",
    emailInvalid: "Por favor, introduce un correo electrónico válido",
    passwordRequired: "La contraseña es obligatoria",
    passwordMinLength: "La contraseña debe tener al menos 6 caracteres",
    passwordMinLengthUpdate: "La contraseña debe tener al menos 8 caracteres",
    confirmPasswordRequired: "Por favor confirma tu contraseña",
    passwordsDoNotMatch: "Las contraseñas no coinciden",
    genderRequired: "Por favor selecciona tu género",
    statusRequired: "Por favor selecciona tu estado",

    // Onboarding Instructions
    howToUseTitle: "Cómo usar el chat",
    onboardingText1:
      "Haz preguntas en tu idioma y recibe respuestas en el mismo idioma. Puedes preguntar sobre tus derechos, procedimientos y servicios disponibles.",
    onboardingText2:
      "El chatbot está disponible 24/7 y puede responder en varios idiomas. Está diseñado para ofrecer información rápida y precisa que te ayude a adaptarte en los Países Bajos.",
    onboardingText3:
      "Recuerda, el chatbot es solo una herramienta de apoyo. Para asuntos complejos o urgentes, busca siempre asesoría legal o de las autoridades correspondientes.",
    startChatting: "Comenzar a chatear",

    // Guide Slides
    guides: [
      {
        title: "Haz preguntas en cualquier momento",
        description:
          "Obtén respuestas instantáneas sobre vivienda, salud, asuntos legales y vida diaria en los Países Bajos, disponible 24/7 en tu idioma.",
        icon: "💬",
      },
      {
        title: "Múltiples idiomas",
        description:
          "Chatea en tu idioma preferido. Nuestro asistente entiende y responde en inglés, neerlandés, árabe, español, francés, turco, ucraniano y ruso.",
        icon: "🌍",
      },
      {
        title: "Información experta",
        description:
          "Accede a información confiable y actualizada sobre leyes, procedimientos y servicios en los Países Bajos para migrantes y solicitantes de asilo.",
        icon: "📚",
      },
      {
        title: "Privacidad y seguridad",
        description:
          "Tus conversaciones son privadas y seguras. Protegemos tu información personal y nunca compartimos tus datos con terceros.",
        icon: "🔒",
      },
      {
        title: "Empieza ahora",
        description:
          "¿Listo para comenzar? Pulsa 'Comenzar a chatear' para hacer tu primera pregunta. Siempre puedes cambiar la configuración de idioma en tu perfil.",
        icon: "🚀",
      },
    ],

    // Navigation
    home: "Inicio",
    chat: "Chat",
    topics: "Temas",
    profile: "Perfil",

    // Chat
    startNewChat: "Iniciar un nuevo chat",
    welcomeToLivenChat: "Bienvenido a Liven Chat",
    chatDescriptionIntro: "Estoy aquí para ayudarte a vivir en los Países Bajos. Pregúntame sobre vivienda, trabajo, servicios gubernamentales o integración.",
    askQuestion: "Haz una pregunta",
    askQuestionDesc: "Obtén respuestas a tus preguntas sobre la vida en los Países Bajos.",
    commonTopics: "Temas comunes",
    typeMessage: "Escribe tu mensaje...",
    chatHistory: "Historial de chat",
    chatLoading: "Pensando...",
    agentConnect: "Conectar con un agente de soporte",
    findAnswers: "Encuentra respuestas a preguntas frecuentes",

    // Topics
    housing: "Vivienda",
    housingDesc: "Ayuda con vivienda y alquiler",
    health: "Salud",
    healthDesc: "Acceso a atención médica y seguros",
    work: "Trabajo",
    workDesc: "Empleos, permisos y orientación laboral",
    legal: "Legal",
    legalDesc: "Residencia, registro y leyes",
    culture: "Cultura",
    cultureDesc: "Tradiciones, idioma y sociedad",
    finance: "Finanzas",
    financeDesc: "Bancos, impuestos y manejo de dinero",
    education: "Educación",
    educationDesc: "Escuelas, cursos y oportunidades de aprendizaje",

    // Profile
    joined: "Se unió",
    account: "Cuenta",
    email: "Correo electrónico",
    settings: "Configuración",
    notifications: "Notificaciones",
    privacy: "Privacidad",
    help: "Ayuda",
    about: "Acerca de",
    updateProfile: "Actualizar perfil",
    changeLanguage: "Cambiar idioma",
    currentLanguage: "Idioma actual",
    selectNewLanguage: "Selecciona un nuevo idioma",
    save: "Guardar",
    cancel: "Cancelar",
    updateEmail: "Actualizar correo electrónico",
    updatePassword: "Actualizar contraseña",
    currentEmail: "Correo electrónico actual",
    newEmail: "Nuevo correo electrónico",
    currentPassword: "Contraseña actual",
    newPassword: "Nueva contraseña",
    confirmNewPassword: "Confirmar nueva contraseña",
    profileUpdated: "Perfil actualizado con éxito",
    languageUpdated: "Idioma actualizado con éxito",

    // Housing
    housing_q1: "¿Cómo me registro en el municipio (BRP)?",
    housing_q2: "¿Qué documentos necesito para alquilar un apartamento?",
    housing_q3: "¿Cómo puedo encontrar vivienda social?",
    housing_q4: "¿Cuáles son mis derechos si el propietario aumenta el alquiler?",
    housing_q5: "¿Cómo solicito la ayuda para vivienda (huurtoeslag)?",

    // Legal
    legal_q1: "¿Cómo solicito un permiso de residencia?",
    legal_q2: "¿Qué es la reunificación familiar y cómo puedo solicitarla?",
    legal_q3: "¿Cuáles son mis derechos mientras espero mi caso de asilo?",
    legal_q4: "¿Dónde puedo obtener asistencia legal gratuita como migrante?",

    // Work
    work_q1: "¿Necesito un permiso de trabajo (TWV)?",
    work_q2: "¿Dónde puedo encontrar vacantes para recién llegados?",
    work_q3: "¿Cuáles son mis derechos como empleado en los Países Bajos?",
    work_q4: "¿Cómo me registro en la oficina de empleo (UWV)?",
    work_q5: "¿Puedo trabajar mientras espero la decisión de asilo?",

    // Healthcare
    healthcare_q1: "¿Cómo me registro con un médico de cabecera (huisarts)?",
    healthcare_q2: "¿Necesito un seguro de salud y cómo lo consigo?",
    healthcare_q3: "¿Qué cubre el seguro de salud básico en los Países Bajos?",
    healthcare_q4: "¿Cómo solicito la ayuda sanitaria (zorgtoeslag)?",

    // Education
    education_q1: "¿Cómo inscribo a mi hijo en la escuela?",
    education_q2: "¿Cuáles son mis opciones para cursos de idioma neerlandés?",
    education_q3: "¿Puedo estudiar en una universidad neerlandesa como recién llegado?",
    education_q4: "¿Existen becas o apoyo financiero para migrantes?",

    // Culture
    culture_q1: "¿Cuáles son algunas tradiciones culturales neerlandesas?",
    culture_q2: "¿Dónde puedo aprender el idioma neerlandés?",
    culture_q3: "¿En qué se diferencian las festividades neerlandesas de las mías?",

    // Finance
    finance_q1: "¿Cómo abro una cuenta bancaria neerlandesa?",
    finance_q2: "¿Cuál es el proceso para obtener un número BSN?",
    finance_q3: "¿Cómo pago impuestos en los Países Bajos?",

    // Theme
    theme: "Tema",
    light: "Claro",
    dark: "Oscuro",
    system: "Sistema",
  },
  fr: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Votre guide multilingue pour vivre aux Pays-Bas.",
    signUp: "S'inscrire",
    logIn: "Se connecter",
    logOut: "Se déconnecter",
    termsText: "En continuant, vous acceptez nos Conditions d'utilisation et notre Politique de confidentialité.",
    sendVerificationCode: "Envoyer le code de vérification",

    // Landing Page
    heroTitle: "Bienvenue dans Liven World",
    heroSubtitle: "Votre assistant multilingue de confiance pour les migrants et demandeurs d'asile aux Pays-Bas",
    heroDescription:
      "Obtenez de l'aide instantanée sur le logement, la santé, les questions juridiques et plus encore - disponible 24/7 dans votre langue.",
    getStartedNow: "Commencer maintenant",
    learnMore: "En savoir plus",

    // Features
    featuresTitle: "Comment nous vous aidons",
    feature1Title: "Support multilingue 24/7",
    feature1Desc: "Obtenez de l'aide à tout moment dans votre langue préférée",
    feature2Title: "Conseils d'experts",
    feature2Desc: "Informations fiables sur le droit, le logement et la santé",
    feature3Title: "Facile à utiliser",
    feature3Desc: "Interface de chat simple conçue pour tous",

    // Welcome & Onboarding
    welcomeTitle: "Bienvenue sur Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "Votre guide multilingue pour naviguer dans la vie aux Pays-Bas",
    welcomeSubtitle: "Votre assistant multilingue pour les migrants et demandeurs d'asile aux Pays-Bas.",
    introRule: "En continuant, vous acceptez nos Conditions d'utilisation et Politique de confidentialité",
    getStarted: "Commencer",

    // Language Selection
    language: "Langue",
    selectLanguage: "Sélectionnez votre langue",
    continue: "Continuer",
    back: "Retour",

    // Profile Setup
    profileSetup: "Configuration du profil",
    tellUsAbout: "Parlez-nous de vous",
    enterEmail: "Entrez votre adresse e-mail",
    profileLogin: "Connexion au profil",
    fullName: "Nom complet",
    nationality: "Nationalité",
    municipality: "Municipalité",
    emailAddress: "Adresse e-mail",
    password: "Mot de passe",
    gender: "Genre",
    status: "Statut",
    next: "Suivant",

    // Gender Options
    male: "Homme",
    female: "Femme",
    other: "Autre",
    preferNotToSay: "Préférer ne pas dire",

    // Status Options
    asylumSeeker: "Demandeur d'asile",
    refugee: "Réfugié",
    migrant: "Migrants",
    student: "Étudiant",

    // Validation Messages
    required: "Ce champ est requis",
    fullNameRequired: "Le nom complet est requis",
    nationalityRequired: "La nationalité est requise",
    municipalityRequired: "La municipalité est requise",
    emailRequired: "L'email est requis",
    emailInvalid: "Veuillez entrer une adresse e-mail valide",
    passwordRequired: "Le mot de passe est requis",
    passwordMinLength: "Le mot de passe doit contenir au moins 6 caractères",
    passwordMinLengthUpdate: "Le mot de passe doit contenir au moins 8 caractères",
    confirmPasswordRequired: "Veuillez confirmer votre mot de passe",
    passwordsDoNotMatch: "Les mots de passe ne correspondent pas",
    genderRequired: "Veuillez sélectionner votre genre",
    statusRequired: "Veuillez sélectionner votre statut",

    // Onboarding Instructions
    howToUseTitle: "Comment utiliser le chat",
    onboardingText1:
      "Posez des questions dans votre langue et obtenez des réponses dans la même langue. Vous pouvez demander vos droits, les procédures et les services disponibles.",
    onboardingText2:
      "Le chatbot est disponible 24/7 et peut répondre en plusieurs langues. Il est conçu pour fournir des informations rapides et précises afin de vous aider à naviguer dans la vie aux Pays-Bas.",
    onboardingText3:
      "Rappelez-vous que le chatbot est un outil pour vous aider. Pour des questions complexes ou urgentes, consultez toujours un professionnel du droit ou les autorités compétentes.",
    startChatting: "Commencer à chatter",

    // Guide Slides
    guides: [
      {
        title: "Posez des questions à tout moment",
        description:
          "Obtenez des réponses instantanées sur le logement, la santé, le droit et la vie quotidienne aux Pays-Bas - disponible 24/7 dans votre langue.",
        icon: "💬",
      },
      {
        title: "Plusieurs langues",
        description:
          "Discutez dans votre langue préférée. Notre assistant IA comprend et répond en anglais, néerlandais, arabe, espagnol, français, turc, ukrainien et russe.",
        icon: "🌍",
      },
      {
        title: "Informations expertes",
        description:
          "Accédez à des informations fiables et à jour sur les lois, procédures et services aux Pays-Bas spécialement conçus pour les migrants et demandeurs d'asile.",
        icon: "📚",
      },
      {
        title: "Confidentialité & Sécurité",
        description:
          "Vos conversations sont privées et sécurisées. Nous protégeons vos informations personnelles et ne partageons jamais vos données avec des tiers.",
        icon: "🔒",
      },
      {
        title: "Commencez maintenant",
        description:
          "Prêt à commencer ? Appuyez sur 'Commencer à chatter' pour poser votre première question. Vous pouvez toujours modifier les paramètres de langue dans votre profil.",
        icon: "🚀",
      },
    ],

    // Navigation
    home: "Accueil",
    chat: "Chat",
    topics: "Sujets",
    profile: "Profil",

    // Chat
    startNewChat: "Commencer un nouveau chat",
    welcomeToLivenChat: "Bienvenue sur Liven Chat",
    chatDescriptionIntro: "Je suis ici pour vous aider à naviguer dans la vie aux Pays-Bas. Posez-moi des questions sur le logement, le travail, les services gouvernementaux ou l'intégration.",
    askQuestion: "Poser une question",
    askQuestionDesc: "Obtenez des réponses à vos questions sur la vie aux Pays-Bas.",
    commonTopics: "Sujets communs",
    typeMessage: "Tapez votre message...",
    chatHistory: "Historique des chats",
    chatLoading: "Réflexion en cours...",
    agentConnect: "Connectez-vous avec un agent de support",
    findAnswers: "Trouvez des réponses aux questions fréquentes",

    // Topics
    housing: "Logement",
    housingDesc: "Aide pour le logement et la location",
    health: "Santé",
    healthDesc: "Accès aux soins de santé et assurances",
    work: "Travail",
    workDesc: "Emplois, permis et orientation professionnelle",
    legal: "Légal",
    legalDesc: "Résidence, enregistrement et droit",
    culture: "Culture",
    cultureDesc: "Traditions, langue et société",
    finance: "Finance",
    financeDesc: "Banques, impôts et gestion de l'argent",
    education: "Éducation",
    educationDesc: "Écoles, cours et opportunités d'apprentissage",

    // Profile
    joined: "Membre depuis",
    account: "Compte",
    email: "E-mail",
    settings: "Paramètres",
    notifications: "Notifications",
    privacy: "Confidentialité",
    help: "Aide",
    about: "À propos",
    updateProfile: "Mettre à jour le profil",
    changeLanguage: "Changer de langue",
    currentLanguage: "Langue actuelle",
    selectNewLanguage: "Sélectionnez une nouvelle langue",
    save: "Enregistrer",
    cancel: "Annuler",
    updateEmail: "Mettre à jour l'email",
    updatePassword: "Mettre à jour le mot de passe",
    currentEmail: "Email actuel",
    newEmail: "Nouvel email",
    currentPassword: "Mot de passe actuel",
    newPassword: "Nouveau mot de passe",
    confirmNewPassword: "Confirmer le nouveau mot de passe",
    profileUpdated: "Profil mis à jour avec succès",
    languageUpdated: "Langue mise à jour avec succès",

    // Housing
    housing_q1: "Comment m'inscrire auprès de la municipalité (BRP) ?",
    housing_q2: "Quels documents sont nécessaires pour louer un appartement ?",
    housing_q3: "Comment puis-je trouver un logement social ?",
    housing_q4: "Quels sont mes droits si le propriétaire augmente le loyer ?",
    housing_q5: "Comment demander l'allocation logement (huurtoeslag) ?",

    // Legal
    legal_q1: "Comment demander un permis de séjour ?",
    legal_q2: "Qu'est-ce que le regroupement familial et comment puis-je en faire la demande ?",
    legal_q3: "Quels sont mes droits pendant l'attente de ma demande d'asile ?",
    legal_q4: "Où puis-je obtenir une aide juridique gratuite en tant que migrant ?",

    // Work
    work_q1: "Ai-je besoin d'un permis de travail (TWV) ?",
    work_q2: "Où puis-je trouver des offres d'emploi pour les nouveaux arrivants ?",
    work_q3: "Quels sont mes droits en tant qu'employé aux Pays-Bas ?",
    work_q4: "Comment m'inscrire à l'UWV (office de l'emploi) ?",
    work_q5: "Puis-je travailler en attendant la décision sur mon asile ?",

    // Healthcare
    healthcare_q1: "Comment m'inscrire auprès d'un médecin généraliste (huisarts) ?",
    healthcare_q2: "Ai-je besoin d'une assurance santé et comment l'obtenir ?",
    healthcare_q3: "Que couvre l'assurance santé de base aux Pays-Bas ?",
    healthcare_q4: "Comment demander l'allocation santé (zorgtoeslag) ?",

    // Education
    education_q1: "Comment inscrire mon enfant à l'école ?",
    education_q2: "Quelles sont mes options pour les cours de néerlandais ?",
    education_q3: "Puis-je étudier dans une université néerlandaise en tant que nouvel arrivant ?",
    education_q4: "Existe-t-il des bourses ou un soutien financier pour les migrants ?",

    // Culture
    culture_q1: "Quelles sont certaines traditions culturelles néerlandaises ?",
    culture_q2: "Où puis-je apprendre le néerlandais ?",
    culture_q3: "En quoi les fêtes néerlandaises diffèrent-elles des miennes ?",

    // Finance
    finance_q1: "Comment ouvrir un compte bancaire néerlandais ?",
    finance_q2: "Quel est le processus pour obtenir un numéro BSN ?",
    finance_q3: "Comment payer les impôts aux Pays-Bas ?",

    // Theme
    theme: "Thème",
    light: "Clair",
    dark: "Sombre",
    system: "Système",
  },
  tr: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Hollanda'da hayatı yönetmek için çok dilli rehberiniz.",
    signUp: "Kayıt Ol",
    logIn: "Giriş Yap",
    logOut: "Çıkış Yap",
    termsText: "Devam ederek Hizmet Şartlarımızı ve Gizlilik Politikamızı kabul etmiş olursunuz.",
    sendVerificationCode: "Doğrulama Kodu Gönder",

    // Landing Page
    heroTitle: "Liven World'e Hoş Geldiniz",
    heroSubtitle: "Hollanda'daki göçmenler ve mülteciler için güvenilir çok dilli asistanınız",
    heroDescription:
      "Konaklama, sağlık, hukuk ve daha fazlası hakkında anında yardım alın - 7/24, kendi dilinizde.",
    getStartedNow: "Hemen Başlayın",
    learnMore: "Daha Fazla Bilgi",

    // Features
    featuresTitle: "Size Nasıl Yardımcı Oluyoruz",
    feature1Title: "7/24 Çok Dilli Destek",
    feature1Desc: "İstediğiniz dilde her zaman yardım alın",
    feature2Title: "Uzman Rehberliği",
    feature2Desc: "Hukuk, konut ve sağlık konularında güvenilir bilgiler",
    feature3Title: "Kolay Kullanım",
    feature3Desc: "Herkes için tasarlanmış basit sohbet arayüzü",

    // Welcome & Onboarding
    welcomeTitle: "Liven Chat'e Hoş Geldiniz",
    introTitle: "Liven Chat",
    welcomeIntro: "Hollanda'da hayatınızı yönetmeniz için çok dilli rehberiniz",
    welcomeSubtitle: "Hollanda'daki göçmenler ve mülteciler için çok dilli asistanınız.",
    introRule: "Devam ederek Hizmet Şartlarımızı ve Gizlilik Politikamızı kabul etmiş olursunuz",
    getStarted: "Başla",

    // Language Selection
    language: "Dil",
    selectLanguage: "Dil Seçin",
    continue: "Devam",
    back: "Geri",

    // Profile Setup
    profileSetup: "Profil Ayarları",
    tellUsAbout: "Bize kendinizden bahsedin",
    enterEmail: "E-posta Adresinizi Girin",
    profileLogin: "Profil Girişi",
    fullName: "Ad Soyad",
    nationality: "Uyruk",
    municipality: "Belediye",
    emailAddress: "E-posta Adresi",
    password: "Şifre",
    gender: "Cinsiyet",
    status: "Durum",
    next: "İleri",

    // Gender Options
    male: "Erkek",
    female: "Kadın",
    other: "Diğer",
    preferNotToSay: "Belirtmek İstemiyorum",

    // Status Options
    asylumSeeker: "Mülteci Başvuru Sahibi",
    refugee: "Mülteci",
    migrant: "Göçmen",
    student: "Öğrenci",

    // Validation Messages
    required: "Bu alan zorunludur",
    fullNameRequired: "Ad soyad zorunludur",
    nationalityRequired: "Uyruk zorunludur",
    municipalityRequired: "Belediye zorunludur",
    emailRequired: "E-posta zorunludur",
    emailInvalid: "Geçerli bir e-posta adresi girin",
    passwordRequired: "Şifre zorunludur",
    passwordMinLength: "Şifre en az 6 karakter olmalıdır",
    passwordMinLengthUpdate: "Şifre en az 8 karakter olmalıdır",
    confirmPasswordRequired: "Lütfen şifrenizi onaylayın",
    passwordsDoNotMatch: "Şifreler eşleşmiyor",
    genderRequired: "Lütfen cinsiyetinizi seçin",
    statusRequired: "Lütfen durumunuzu seçin",

    // Onboarding Instructions
    howToUseTitle: "Sohbeti nasıl kullanırım",
    onboardingText1:
      "Sorularınızı kendi dilinizde sorun ve yanıtları aynı dilde alın. Haklarınız, prosedürler ve mevcut hizmetler hakkında sorabilirsiniz.",
    onboardingText2:
      "Chatbot 7/24 kullanılabilir ve birden fazla dilde sorulara cevap verebilir. Hollanda'da durumunuzu yönetmenize yardımcı olmak için hızlı ve doğru bilgi sunmak üzere tasarlanmıştır.",
    onboardingText3:
      "Unutmayın, chatbot yalnızca bir yardım aracıdır. Karmaşık veya acil durumlar için her zaman bir hukuk uzmanına veya ilgili yetkililere danışın.",
    startChatting: "Sohbete Başla",

    // Guide Slides
    guides: [
      {
        title: "İstediğiniz Zaman Sorular Sorun",
        description:
          "Konut, sağlık, hukuk ve Hollanda'daki günlük yaşam hakkında anında yanıt alın - 7/24, kendi dilinizde.",
        icon: "💬",
      },
      {
        title: "Birden Fazla Dil",
        description:
          "Tercih ettiğiniz dilde sohbet edin. Asistanımız İngilizce, Hollandaca, Arapça, İspanyolca, Fransızca, Türkçe, Ukraynaca ve Rusça anlayabilir ve yanıtlayabilir.",
        icon: "🌍",
      },
      {
        title: "Uzman Bilgisi",
        description:
          "Göçmenler ve mülteciler için özel olarak tasarlanmış Hollanda yasaları, prosedürleri ve hizmetleri hakkında güvenilir ve güncel bilgi edinin.",
        icon: "📚",
      },
      {
        title: "Gizlilik ve Güvenlik",
        description:
          "Konuşmalarınız gizli ve güvenlidir. Kişisel bilgilerinizi koruyor ve verilerinizi üçüncü taraflarla paylaşmıyoruz.",
        icon: "🔒",
      },
      {
        title: "Başlamaya Hazır",
        description:
          "Başlamaya hazır mısınız? İlk soruyu sormak için 'Sohbete Başla'ya dokunun. Dil ayarlarınızı her zaman profilinizden değiştirebilirsiniz.",
        icon: "🚀",
      },
    ],

    // Navigation
    home: "Ana Sayfa",
    chat: "Sohbet",
    topics: "Konu Başlıkları",
    profile: "Profil",

    // Chat
    startNewChat: "Yeni bir sohbet başlat",
    welcomeToLivenChat: "Liven Chat'e Hoş Geldiniz",
    chatDescriptionIntro:
      "Hollanda'da hayatınızı yönetmenize yardımcı olmak için buradayım. Konut, iş, devlet hizmetleri veya entegrasyon hakkında bana sorabilirsiniz.",
    askQuestion: "Bir soru sor",
    askQuestionDesc: "Hollanda'da yaşam hakkında sorularınıza yanıt alın.",
    commonTopics: "Yaygın konular",
    typeMessage: "Mesajınızı yazın...",
    chatHistory: "Sohbet Geçmişi",
    chatLoading: "Düşünüyor...",
    agentConnect: "Destek temsilcisi ile bağlan",
    findAnswers: "Sık Sorulan Soruların Cevaplarını Bul",

    // Topics
    housing: "Konut",
    housingDesc: "Konut ve kira ile ilgili yardım",
    health: "Sağlık",
    healthDesc: "Sağlık hizmetlerine ve sigortaya erişim",
    work: "İş",
    workDesc: "İşler, izinler ve kariyer rehberliği",
    legal: "Hukuk",
    legalDesc: "İkamet, kayıt ve yasalar",
    culture: "Kültür",
    cultureDesc: "Gelenekler, dil ve toplum",
    finance: "Finans",
    financeDesc: "Bankacılık, vergiler ve para yönetimi",
    education: "Eğitim",
    educationDesc: "Okullar, kurslar ve öğrenim fırsatları",

    // Profile
    joined: "Katılım Tarihi",
    account: "Hesap",
    email: "E-posta",
    settings: "Ayarlar",
    notifications: "Bildirimler",
    privacy: "Gizlilik",
    help: "Yardım",
    about: "Hakkında",
    updateProfile: "Profili Güncelle",
    changeLanguage: "Dili Değiştir",
    currentLanguage: "Mevcut Dil",
    selectNewLanguage: "Yeni Dil Seçin",
    save: "Kaydet",
    cancel: "İptal",
    updateEmail: "E-posta Güncelle",
    updatePassword: "Şifre Güncelle",
    currentEmail: "Mevcut E-posta",
    newEmail: "Yeni E-posta",
    currentPassword: "Mevcut Şifre",
    newPassword: "Yeni Şifre",
    confirmNewPassword: "Yeni Şifreyi Onayla",
    profileUpdated: "Profil başarıyla güncellendi",
    languageUpdated: "Dil başarıyla güncellendi",

    // Housing
    housing_q1: "Belediyede (BRP) nasıl kayıt olabilirim?",
    housing_q2: "Bir daire kiralamak için hangi belgelere ihtiyacım var?",
    housing_q3: "Sosyal konutları nasıl bulabilirim?",
    housing_q4: "Ev sahibim kirası artırırsa haklarım nelerdir?",
    housing_q5: "Konut yardımı (huurtoeslag) için nasıl başvurabilirim?",

    // Legal
    legal_q1: "İkamet izni için nasıl başvurabilirim?",
    legal_q2: "Aile birleşimi nedir ve nasıl başvurabilirim?",
    legal_q3: "Mülteci başvurum beklerken haklarım nelerdir?",
    legal_q4: "Göçmen olarak ücretsiz hukuki yardım nereden alabilirim?",

    // Work
    work_q1: "Çalışma iznine (TWV) ihtiyacım var mı?",
    work_q2: "Yeni gelenler için iş ilanlarını nereden bulabilirim?",
    work_q3: "Hollanda'da çalışan olarak haklarım nelerdir?",
    work_q4: "UWV'ye (İş Kurumu) nasıl kayıt olabilirim?",
    work_q5: "Asil kararı beklerken çalışabilir miyim?",

    // Healthcare
    healthcare_q1: "Huisarts (aile hekimi) ile nasıl kayıt olabilirim?",
    healthcare_q2: "Sağlık sigortasına ihtiyacım var mı ve nasıl alabilirim?",
    healthcare_q3: "Hollanda temel sağlık sigortası neleri kapsar?",
    healthcare_q4: "Sağlık yardımı (zorgtoeslag) için nasıl başvurabilirim?",

    // Education
    education_q1: "Çocuğumu okula nasıl kaydettirebilirim?",
    education_q2: "Hollandaca dil kursları için seçeneklerim nelerdir?",
    education_q3: "Yeni gelen biri olarak Hollanda üniversitesinde eğitim alabilir miyim?",
    education_q4: "Göçmenler için burs veya maddi destek var mı?",

    // Culture
    culture_q1: "Bazı Hollanda kültürel gelenekleri nelerdir?",
    culture_q2: "Hollandaca öğrenebileceğim yerler neresi?",
    culture_q3: "Hollanda tatilleri benimkilerden nasıl farklıdır?",

    // Finance
    finance_q1: "Hollanda'da banka hesabı nasıl açılır?",
    finance_q2: "BSN numarası almak için süreç nedir?",
    finance_q3: "Hollanda'da vergiler nasıl ödenir?",

    // Theme
    theme: "Tema",
    light: "Açık",
    dark: "Koyu",
    system: "Sistem",
  },
  uk: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Ваш багатомовний гід по життю в Нідерландах.",
    signUp: "Зареєструватися",
    logIn: "Увійти",
    logOut: "Вийти",
    termsText: "Продовжуючи, ви погоджуєтесь з нашими Умовами надання послуг та Політикою конфіденційності.",
    sendVerificationCode: "Надіслати код підтвердження",

    // Landing Page
    heroTitle: "Ласкаво просимо до Liven World",
    heroSubtitle: "Ваш надійний багатомовний помічник для мігрантів та шукачів притулку в Нідерландах",
    heroDescription:
      "Отримуйте миттєву допомогу з житлом, охороною здоров'я, юридичними питаннями та іншим — доступно 24/7 вашою мовою.",
    getStartedNow: "Почати зараз",
    learnMore: "Дізнатися більше",

    // Features
    featuresTitle: "Як ми вам допомагаємо",
    feature1Title: "Багатомовна підтримка 24/7",
    feature1Desc: "Отримуйте допомогу будь-коли вашою мовою",
    feature2Title: "Експертні поради",
    feature2Desc: "Надійна інформація про право, житло та охорону здоров'я",
    feature3Title: "Легко використовувати",
    feature3Desc: "Простий чат-інтерфейс, розроблений для всіх",

    // Welcome & Onboarding
    welcomeTitle: "Ласкаво просимо до Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "Ваш багатомовний гід по навігації в житті Нідерландів",
    welcomeSubtitle: "Ваш багатомовний помічник для мігрантів та шукачів притулку в Нідерландах.",
    introRule: "Продовжуючи, ви погоджуєтесь з нашими Умовами надання послуг та Політикою конфіденційності",
    getStarted: "Почати",

    // Language Selection
    language: "Мова",
    selectLanguage: "Виберіть вашу мову",
    continue: "Продовжити",
    back: "Назад",

    // Profile Setup
    profileSetup: "Налаштування профілю",
    tellUsAbout: "Розкажіть про себе",
    enterEmail: "Введіть вашу електронну адресу",
    profileLogin: "Вхід до профілю",
    fullName: "Повне ім’я",
    nationality: "Національність",
    municipality: "Муніципалітет",
    emailAddress: "Електронна адреса",
    password: "Пароль",
    gender: "Стать",
    status: "Статус",
    next: "Далі",

    // Gender Options
    male: "Чоловік",
    female: "Жінка",
    other: "Інше",
    preferNotToSay: "Не хочу вказувати",

    // Status Options
    asylumSeeker: "Шукач притулку",
    refugee: "Біженець",
    migrant: "Мігрант",
    student: "Студент",

    // Validation Messages
    required: "Це поле обов'язкове",
    fullNameRequired: "Повне ім’я обов’язкове",
    nationalityRequired: "Національність обов’язкова",
    municipalityRequired: "Муніципалітет обов’язковий",
    emailRequired: "Електронна адреса обов’язкова",
    emailInvalid: "Будь ласка, введіть дійсну електронну адресу",
    passwordRequired: "Пароль обов’язковий",
    passwordMinLength: "Пароль повинен містити щонайменше 6 символів",
    passwordMinLengthUpdate: "Пароль повинен містити щонайменше 8 символів",
    confirmPasswordRequired: "Будь ласка, підтвердіть ваш пароль",
    passwordsDoNotMatch: "Паролі не співпадають",
    genderRequired: "Будь ласка, оберіть вашу стать",
    statusRequired: "Будь ласка, оберіть ваш статус",

    // Onboarding Instructions
    howToUseTitle: "Як користуватися чатом",
    onboardingText1:
      "Ставте запитання вашою мовою та отримуйте відповіді тією ж мовою. Ви можете дізнатися про свої права, процедури та доступні послуги.",
    onboardingText2:
      "Чат-бот доступний 24/7 та може відповідати багатьма мовами. Він розроблений для швидкого та точного надання інформації, щоб допомогти вам у Нідерландах.",
    onboardingText3:
      "Пам’ятайте, чат-бот — це інструмент допомоги. Для складних або термінових питань завжди консультуйтеся з юристом або відповідними органами.",
    startChatting: "Почати чат",

    // Guide Slides
    guides: [
      {
        title: "Ставте запитання у будь-який час",
        description:
          "Отримуйте миттєві відповіді про житло, охорону здоров'я, право та повсякденне життя в Нідерландах — доступно 24/7 вашою мовою.",
        icon: "💬"
      },
      {
        title: "Багатомовність",
        description:
          "Спілкуйтеся вашою мовою. Наш асистент AI розуміє та відповідає англійською, голландською, арабською, іспанською, французькою, турецькою, українською та російською.",
        icon: "🌍"
      },
      {
        title: "Експертна інформація",
        description:
          "Отримуйте надійну, актуальну інформацію про закони, процедури та послуги в Нідерландах, спеціально для мігрантів та шукачів притулку.",
        icon: "📚"
      },
      {
        title: "Конфіденційність та безпека",
        description:
          "Ваші розмови приватні та захищені. Ми оберігаємо вашу особисту інформацію і ніколи не передаємо її третім сторонам.",
        icon: "🔒"
      },
      {
        title: "Почніть зараз",
        description:
          "Готові почати? Натисніть 'Почати чат', щоб задати перше запитання. Ви завжди можете змінити мовні налаштування у своєму профілі.",
        icon: "🚀"
      }
    ],

    // Navigation
    home: "Головна",
    chat: "Чат",
    topics: "Теми",
    profile: "Профіль",

    // Chat
    startNewChat: "Почати новий чат",
    welcomeToLivenChat: "Ласкаво просимо до Liven Chat",
    chatDescriptionIntro:
      "Я тут, щоб допомогти вам орієнтуватися в житті Нідерландів. Запитуйте про житло, роботу, державні послуги або інтеграцію.",
    askQuestion: "Задати питання",
    askQuestionDesc: "Отримайте відповіді на ваші запитання про життя в Нідерландах.",
    commonTopics: "Поширені теми",
    typeMessage: "Введіть ваше повідомлення...",
    chatHistory: "Історія чату",
    chatLoading: "Думка ...",
    agentConnect: "Зв'язатися з агентом підтримки",
    findAnswers: "Знайдіть відповіді на поширені запитання",

    // Topics
    housing: "Житло",
    housingDesc: "Допомога з житлом та орендою",
    health: "Здоров’я",
    healthDesc: "Доступ до медичних послуг та страхування",
    work: "Робота",
    workDesc: "Вакансії, дозволи та кар'єрна підтримка",
    legal: "Юридичні питання",
    legalDesc: "Реєстрація, проживання та закон",
    culture: "Культура",
    cultureDesc: "Традиції, мова та суспільство",
    finance: "Фінанси",
    financeDesc: "Банки, податки та управління грошима",
    education: "Освіта",
    educationDesc: "Школи, курси та можливості навчання",

    // Profile
    joined: "Приєднався",
    account: "Акаунт",
    email: "Електронна пошта",
    settings: "Налаштування",
    notifications: "Сповіщення",
    privacy: "Конфіденційність",
    help: "Допомога",
    about: "Про нас",
    updateProfile: "Оновити профіль",
    changeLanguage: "Змінити мову",
    currentLanguage: "Поточна мова",
    selectNewLanguage: "Вибрати нову мову",
    save: "Зберегти",
    cancel: "Скасувати",
    updateEmail: "Оновити електронну пошту",
    updatePassword: "Оновити пароль",
    currentEmail: "Поточна електронна пошта",
    newEmail: "Нова електронна пошта",
    currentPassword: "Поточний пароль",
    newPassword: "Новий пароль",
    confirmNewPassword: "Підтвердьте новий пароль",
    profileUpdated: "Профіль успішно оновлено",
    languageUpdated: "Мову успішно оновлено",

    // Housing
    housing_q1: "Як зареєструватися в муніципалітеті (BRP)?",
    housing_q2: "Які документи потрібні для оренди квартири?",
    housing_q3: "Як знайти соціальне житло?",
    housing_q4: "Які мої права, якщо орендодавець підвищує орендну плату?",
    housing_q5: "Як подати заявку на житлову допомогу (huurtoeslag)?",

    // Legal
    legal_q1: "Як подати заявку на вид на проживання?",
    legal_q2: "Що таке возз’єднання сім’ї та як на нього подати?",
    legal_q3: "Які мої права під час очікування розгляду прохання про притулок?",
    legal_q4: "Де можна отримати безкоштовну юридичну допомогу як мігрант?",

    // Work
    work_q1: "Мені потрібен дозвіл на роботу (TWV)?",
    work_q2: "Де можна знайти вакансії для новоприбулих?",
    work_q3: "Які мої права як працівника в Нідерландах?",
    work_q4: "Як зареєструватися в UWV (служба зайнятості)?",
    work_q5: "Чи можу я працювати, очікуючи рішення про притулок?",

    // Healthcare
    healthcare_q1: "Як зареєструватися у huisarts (лікар загальної практики)?",
    healthcare_q2: "Мені потрібна медична страховка і як її отримати?",
    healthcare_q3: "Що покриває базова медична страховка в Нідерландах?",
    healthcare_q4: "Як подати заявку на медичну допомогу (zorgtoeslag)?",

    // Education
    education_q1: "Як записати дитину до школи?",
    education_q2: "Які є варіанти курсів голландської мови?",
    education_q3: "Чи можу я навчатися в голландському університеті як новоприбулий?",
    education_q4: "Чи існують стипендії або фінансова підтримка для мігрантів?",

    // Culture
    culture_q1: "Які деякі голландські культурні традиції?",
    culture_q2: "Де я можу вивчати голландську мову?",
    culture_q3: "Чим голландські свята відрізняються від моїх?",

    // Finance
    finance_q1: "Як відкрити банківський рахунок у Нідерландах?",
    finance_q2: "Який процес отримання BSN номера?",
    finance_q3: "Як сплачувати податки в Нідерландах?",

    // Theme
    theme: "Тема",
    light: "Світла",
    dark: "Темна",
    system: "Системна"
  },
  ru: {
    // Landing & Auth
    appTitle: "Liven Chat",
    appSubtitle: "Ваш многоязычный гид по жизни в Нидерландах.",
    signUp: "Зарегистрироваться",
    logIn: "Войти",
    logOut: "Выйти",
    termsText: "Продолжая, вы соглашаетесь с нашими Условиями обслуживания и Политикой конфиденциальности.",
    sendVerificationCode: "Отправить код подтверждения",

    // Landing Page
    heroTitle: "Добро пожаловать в Liven World",
    heroSubtitle: "Ваш надежный многоязычный помощник для мигрантов и соискателей убежища в Нидерландах",
    heroDescription:
      "Получайте мгновенную помощь по вопросам жилья, здравоохранения, юридических вопросов и не только — доступно 24/7 на вашем языке.",
    getStartedNow: "Начать сейчас",
    learnMore: "Узнать больше",

    // Features
    featuresTitle: "Как мы вам помогаем",
    feature1Title: "Многоязычная поддержка 24/7",
    feature1Desc: "Получайте помощь в любое время на вашем языке",
    feature2Title: "Экспертные рекомендации",
    feature2Desc: "Надежная информация по юридическим, жилищным и медицинским вопросам",
    feature3Title: "Просто в использовании",
    feature3Desc: "Простой чат-интерфейс, разработанный для всех",

    // Welcome & Onboarding
    welcomeTitle: "Добро пожаловать в Liven Chat",
    introTitle: "Liven Chat",
    welcomeIntro: "Ваш многоязычный гид по жизни в Нидерландах",
    welcomeSubtitle: "Ваш многоязычный помощник для мигрантов и соискателей убежища в Нидерландах.",
    introRule: "Продолжая, вы соглашаетесь с нашими Условиями обслуживания и Политикой конфиденциальности",
    getStarted: "Начать",

    // Language Selection
    language: "Язык",
    selectLanguage: "Выберите ваш язык",
    continue: "Продолжить",
    back: "Назад",

    // Profile Setup
    profileSetup: "Настройка профиля",
    tellUsAbout: "Расскажите о себе",
    enterEmail: "Введите вашу электронную почту",
    profileLogin: "Вход в профиль",
    fullName: "Полное имя",
    nationality: "Национальность",
    municipality: "Муниципалитет",
    emailAddress: "Электронная почта",
    password: "Пароль",
    gender: "Пол",
    status: "Статус",
    next: "Далее",

    // Gender Options
    male: "Мужской",
    female: "Женский",
    other: "Другое",
    preferNotToSay: "Предпочитаю не указывать",

    // Status Options
    asylumSeeker: "Соискатель убежища",
    refugee: "Беженец",
    migrant: "Мигрант",
    student: "Студент",

    // Validation Messages
    required: "Это поле обязательно",
    fullNameRequired: "Полное имя обязательно",
    nationalityRequired: "Национальность обязательна",
    municipalityRequired: "Муниципалитет обязателен",
    emailRequired: "Электронная почта обязательна",
    emailInvalid: "Пожалуйста, введите корректный адрес электронной почты",
    passwordRequired: "Пароль обязателен",
    passwordMinLength: "Пароль должен содержать не менее 6 символов",
    passwordMinLengthUpdate: "Пароль должен содержать не менее 8 символов",
    confirmPasswordRequired: "Пожалуйста, подтвердите ваш пароль",
    passwordsDoNotMatch: "Пароли не совпадают",
    genderRequired: "Пожалуйста, выберите ваш пол",
    statusRequired: "Пожалуйста, выберите ваш статус",

    // Onboarding Instructions
    howToUseTitle: "Как использовать чат",
    onboardingText1:
      "Задавайте вопросы на вашем языке и получайте ответы на том же языке. Вы можете узнать о своих правах, процедурах и доступных услугах.",
    onboardingText2:
      "Чат-бот доступен 24/7 и может отвечать на нескольких языках. Он разработан для быстрой и точной информации, чтобы помочь вам в Нидерландах.",
    onboardingText3:
      "Помните, чат-бот — это инструмент помощи. Для сложных или срочных вопросов всегда консультируйтесь с юристом или соответствующими органами.",
    startChatting: "Начать чат",

    // Guide Slides
    guides: [
      {
        title: "Задавайте вопросы в любое время",
        description:
          "Получайте мгновенные ответы по вопросам жилья, здравоохранения, права и повседневной жизни в Нидерландах — доступно 24/7 на вашем языке.",
        icon: "💬"
      },
      {
        title: "Многоязычность",
        description:
          "Общайтесь на вашем языке. Наш AI-помощник понимает и отвечает на английском, голландском, арабском, испанском, французском, турецком, украинском и русском.",
        icon: "🌍"
      },
      {
        title: "Экспертная информация",
        description:
          "Доступ к надежной и актуальной информации о законах, процедурах и услугах в Нидерландах, специально для мигрантов и соискателей убежища.",
        icon: "📚"
      },
      {
        title: "Конфиденциальность и безопасность",
        description:
          "Ваши разговоры конфиденциальны и защищены. Мы сохраняем ваши личные данные и никогда не передаем их третьим лицам.",
        icon: "🔒"
      },
      {
        title: "Начните сейчас",
        description:
          "Готовы начать? Нажмите 'Начать чат', чтобы задать первый вопрос. Вы всегда можете изменить языковые настройки в профиле.",
        icon: "🚀"
      }
    ],

    // Navigation
    home: "Главная",
    chat: "Чат",
    topics: "Темы",
    profile: "Профиль",

    // Chat
    startNewChat: "Начать новый чат",
    welcomeToLivenChat: "Добро пожаловать в Liven Chat",
    chatDescriptionIntro:
      "Я здесь, чтобы помочь вам ориентироваться в жизни в Нидерландах. Спросите меня о жилье, работе, государственных услугах или интеграции.",
    askQuestion: "Задать вопрос",
    askQuestionDesc: "Получайте ответы на ваши вопросы о жизни в Нидерландах.",
    commonTopics: "Популярные темы",
    typeMessage: "Введите сообщение...",
    chatHistory: "История чата",
    chatLoading: "Думаю ...",
    agentConnect: "Связаться с агентом поддержки",
    findAnswers: "Найдите ответы на часто задаваемые вопросы",

    // Topics
    housing: "Жильё",
    housingDesc: "Помощь с жильём и арендой",
    health: "Здоровье",
    healthDesc: "Доступ к медицинским услугам и страховке",
    work: "Работа",
    workDesc: "Вакансии, разрешения и карьерная поддержка",
    legal: "Юридические вопросы",
    legalDesc: "Регистрация, проживание и закон",
    culture: "Культура",
    cultureDesc: "Традиции, язык и общество",
    finance: "Финансы",
    financeDesc: "Банки, налоги и управление деньгами",
    education: "Образование",
    educationDesc: "Школы, курсы и возможности обучения",

    // Profile
    joined: "Присоединился",
    account: "Аккаунт",
    email: "Электронная почта",
    settings: "Настройки",
    notifications: "Уведомления",
    privacy: "Конфиденциальность",
    help: "Помощь",
    about: "О нас",
    updateProfile: "Обновить профиль",
    changeLanguage: "Сменить язык",
    currentLanguage: "Текущий язык",
    selectNewLanguage: "Выберите новый язык",
    save: "Сохранить",
    cancel: "Отмена",
    updateEmail: "Обновить email",
    updatePassword: "Обновить пароль",
    currentEmail: "Текущий email",
    newEmail: "Новый email",
    currentPassword: "Текущий пароль",
    newPassword: "Новый пароль",
    confirmNewPassword: "Подтвердите новый пароль",
    profileUpdated: "Профиль успешно обновлен",
    languageUpdated: "Язык успешно обновлен",

    // Housing
    housing_q1: "Как зарегистрироваться в муниципалитете (BRP)?",
    housing_q2: "Какие документы нужны для аренды квартиры?",
    housing_q3: "Как найти социальное жильё?",
    housing_q4: "Какие мои права, если арендодатель повышает аренду?",
    housing_q5: "Как подать заявку на жилищное пособие (huurtoeslag)?",

    // Legal
    legal_q1: "Как подать заявку на вид на жительство?",
    legal_q2: "Что такое воссоединение семьи и как подать заявку?",
    legal_q3: "Какие мои права во время рассмотрения запроса на убежище?",
    legal_q4: "Где можно получить бесплатную юридическую помощь для мигрантов?",

    // Work
    work_q1: "Нужен ли мне разрешение на работу (TWV)?",
    work_q2: "Где найти вакансии для новоприбывших?",
    work_q3: "Какие мои права как работника в Нидерландах?",
    work_q4: "Как зарегистрироваться в UWV (служба занятости)?",
    work_q5: "Могу ли я работать, ожидая решения по убежищу?",

    // Healthcare
    healthcare_q1: "Как зарегистрироваться у huisarts (врач общей практики)?",
    healthcare_q2: "Нужна ли медицинская страховка и как её получить?",
    healthcare_q3: "Что покрывает базовая медицинская страховка в Нидерландах?",
    healthcare_q4: "Как подать заявку на пособие на здравоохранение (zorgtoeslag)?",

    // Education
    education_q1: "Как записать ребёнка в школу?",
    education_q2: "Какие есть варианты курсов голландского языка?",
    education_q3: "Могу ли я учиться в голландском университете как новоприбывший?",
    education_q4: "Существуют ли стипендии или финансовая поддержка для мигрантов?",

    // Culture
    culture_q1: "Какие есть голландские культурные традиции?",
    culture_q2: "Где можно изучать голландский язык?",
    culture_q3: "Чем голландские праздники отличаются от моих?",

    // Finance
    finance_q1: "Как открыть голландский банковский счёт?",
    finance_q2: "Как получить BSN номер?",
    finance_q3: "Как платить налоги в Нидерландах?",

    // Theme
    theme: "Тема",
    light: "Светлая",
    dark: "Тёмная",
    system: "Системная"
  }
}

export type Language = keyof typeof translations
export type TranslationKey = keyof typeof translations.en

export const languages = [
  { code: "en" as Language, name: "English" },
  { code: "nl" as Language, name: "Dutch" },
  { code: "ar" as Language, name: "Arabic" },
  { code: "es" as Language, name: "Spanish" },
  { code: "fr" as Language, name: "French" },
  { code: "tr" as Language, name: "Turkish" },
  { code: "uk" as Language, name: "Ukrainian" },
  { code: "ru" as Language, name: "Russian" },
]
